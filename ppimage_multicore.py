...
def preprocess(fileName, wh):
        ...
        # Return false if the image was too small.
        if minDim < wh:
                return False
        else:
                # Here, we crop in the longer direction to make a square.
                if iW == minDim:
                        # Crop along Y
                        offset = (iH - minDim) / 2
        ...
        # Now we scale the image, after it is already square.
        img.thumbnail( (wh, wh), Image.ANTIALIAS )
        assert(img.size[0] == wh and img.size[1] == wh) # Make sure we have the correct dimensions.
